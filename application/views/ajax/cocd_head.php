	...
	<form name="codeHead" id="codeHead">
		...
		<div class="register_form">
			...
			<div class="bcont">
				...
				<!--button type="button" data-code="<?php echo $data->CODE;?>" class="delBtn blue_btn">삭제</button-->
				<?php
				}else{	
				?>
...
$("#CODE").change(function(){
	...
	$.post("<?php echo base_url('mdm/ajax_cocdHaedchk');?>",{code:$this.val()},function(data){
        if(data.state == "N"){
			$this.focus();
			chkHeadCode = true;
			alert(data.msg);
...
$(".modBtn").on("click",function(){
	...
	$.ajax({
		url  : "<?php echo base_url('mdm/set_cocd_HeadUpdate')?>",
		type : "POST",
		data : formData,
		//asynsc : true,
		cache  : false,
