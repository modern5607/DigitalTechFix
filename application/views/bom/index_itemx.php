	...
	<div>
		<form id="items_formupdate">
			...
			?>
				...
				<?php
				foreach($MSAB as $row){
					$selected1 = ($str['mscode'] == $row->D_CODE)?"selected":"";
				?>
...
function ajax_container(idx){
	...
	$.ajax({
		url : "<?php echo base_url('mdm/index_ajax/')?>",
		type : "post",
		data : {idx:idx},
		dataType : "html",
...
function itemsformchk(f){
	...
	var line1 = $("select[name='M_LINE']");
	var line2 = $("select[name='2ND_LINE']");
	var line3 = $("select[name='3ND_LINE']");
	...
	$.ajax({
		url  : "<?php echo base_url('mdm/itemsUpdate')?>",
		type : "POST",
		data : formData,
		cache  : false,
		contentType : false,
