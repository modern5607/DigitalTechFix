		...
		<header>
			...
			<div style="float:left;">
				<form id="items_formupdate">
					<label for="m1">전체</label><input type="radio" name="matCount" value="" id="m1" <?php echo (!empty($str['matCount']))?($str['matCount'] == "")?"checked":"":"checked";?>>
					<label for="m2">수량차이</label><input type="radio" name="matCount" value="s" id="m2" <?php echo ($str['matCount'] == "s")?"checked":"";?>>
...
$(".setComponent").on("click",function(){
	...
	if(confirm("일괄수정을 진행하시겠습니까?") !== false){
		$.ajax({
...
$(".mod_material").on("click",function(){
	...
	if(confirm('재고량을 수정하시겠습니까?') !== false){
		$.ajax({
			url : "<?php echo base_url('mat/ajax_setComponent')?>",
			type : "post",
...
$(".print_detail").on("click",function(){
	if(confirm('해당정보를 엑셀다운로드 하시겠습니까?') !== false){
		location.href = "<?php echo base_url('/mat/materials_excelDown')?>";
	}
});
